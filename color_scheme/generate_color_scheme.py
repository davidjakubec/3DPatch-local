from matplotlib import cm
from sys import argv
from numpy import linspace

cmap = []

for i in range(256):
    r, g, b, a = cm.inferno(i)
    cmap.append([round(float(r), 6), round(float(g), 6), round(float(b), 6)])

cmap.reverse()

number_of_colors = int(argv[1])
boundaries = linspace(0, 255, number_of_colors + 1)
color_ids = []

for i in range(0, len(boundaries) - 1):
    color_id = (boundaries[i] + boundaries[i + 1]) / 2
    color_ids.append(int(round(color_id)))

color_scheme = []

for color_id in color_ids:
    for i in range(256 // number_of_colors):
        color_scheme.append(cmap[color_id])

for i in range(256 % number_of_colors):
    color_scheme.append(cmap[color_ids[-1]])

print(len(set([tuple(i) for i in color_scheme])), len(color_scheme))

with open('color_scheme', encoding = 'UTF-8', mode = 'w') as file:
    for color in color_scheme:
        file.write(' '.join([str(i) for i in color]) + '\n')
